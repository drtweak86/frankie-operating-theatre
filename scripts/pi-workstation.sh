...
if command -v >/dev/null 2>&1 && [[ ! -f "${PROFILE_DIR}//config.jsonc" ]]; then
 sudo -u "$USER_NAME" --gen-config >/dev/null 2>&1 || true
fi
...
install_fastfetch() {
 ...
 echo "[+] Installing (Ubuntu ARM)"
 tmp=/tmp/.tar.gz
 curl -fsSL -o "$tmp" \
 https://github.com/-cli//releases/latest/download/-linux-aarch64-polyfilled.tar.gz || {
 ...
 elif [ -x /usr/local/bin/-linux-aarch64-polyfilled/usr/bin/ ]; then
 sudo ln -sf /usr/local/bin/-linux-aarch64-polyfilled/usr/bin/ /usr/local/bin/
 fi

 if ! command -v >/dev/null 2>&1; then
 echo "[-] not found on PATH after install"; return 1
 fi
...
install_fastfetch() {
  if command -v fastfetch >/dev/null 2>&1; then
    echo "[✓] fastfetch already installed"; return
  fi
  ...
  curl -fsSL -o "$tmp" \
    https://github.com/fastfetch-cli/fastfetch/releases/latest/download/fastfetch-linux-aarch64-polyfilled.tar.gz || {
      echo "[-] Download failed"; return 1; }
  # Tar has a /usr layout inside — extract to root
  sudo tar -xzf "$tmp" -C / || { echo "[-] Extract failed"; return 1; }

  # Ensure it's on PATH even if landed under /usr/local/bin/fastfetch-linux-*/usr/bin
